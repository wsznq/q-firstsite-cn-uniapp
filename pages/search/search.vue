	...
	<view class="index-content">
		...
		<view class="index-coupon has-bg-white has-pd-10 top_30">
			<view class="goods-list" v-if="couponlist.length > 0&&fromTabIndex==1" style="padding-top: 230upx">
				<orange-goods-card-home v-for="(item,index) in couponlist" :index="index%2" :logo="pddlogo" :tkmoney='item.tkmoney'
				 ...
				 :itemtitle="item.itemtitle" :itemprice='"¥"+item.itemprice' :itemsale='"已售 "+item.salesTip' :itemendprice='item.itemendprice'
		...
		<view class="index-coupon has-bg-white has-pd-10 top_30">
			<view class="goods-list" v-if="couponlist.length > 0&&fromTabIndex==0" style="padding-top: 172upx">
				<orange-goods-card-home v-for="(item,index) in couponlist" :index="index%2" :logo="item.shoptype=='1'?logo:taobao"
	...
	export default {
		...
		methods: {
				...
				this.$Request.get(this.$Api.common.couponlist,{
					...
					if (res.Code === 1) {
						...
						res.Message.forEach(d => {
							...
							d.tkmoneys = ((d.itemendprice * (d.tkmoney * 0.01)) * 0.7).toFixed(2);
							if (grade) {
								d.tkmoney = ((d.itemendprice * (d.tkmoney * 0.01)) * parseFloat(grade)).toFixed(2);
							} else {
								d.tkmoney = ((d.itemendprice * (d.tkmoney * 0.01)) * 0.3).toFixed(2);
							}
			...
			loadPddCouponList: function(type) {
				this.loadingType = 1;
				this.$Request.get(this.$Api.common.couponlistsearch_pdd,{
					KeyWords: this.keywords,
					PageNo: this.page,
					Position: this.pddSort,
					DeviceValue: this.deviceValue
				}).then(res => {
					console.log(res);
					this.loadingType = 0;
