		...
		<view class="index-goods" v-if="loadGoods">
			...
			<view class="goods_info" style="padding-top: 6px;padding-bottom: 10px">
				<view style="padding: 8px">
					<image style="height: 14px;width:12px;margin-top: 4px;margin-right: 4px" src="../../static/img/pinduoduo.png"></image>
...
<script>
	...
	export default {
		...
		methods: {
			...
			loadGoodsInfo: function() {
				...
				this.$Request.get(this.$Api.goods.iteminfo_pdd,{
					...
					if (res.Code === 1) {
						...
						that.grade = grade;
						try {
							let isExist = this.$queue.isExistPdd(this.collectKey, that.goods.itemid);
							if (isExist === true) {
							...
							let isExists = this.$queue.isExistPdd(this.footprintKey, that.goods.itemid);
							if (isExists === false) {
								this.$queue.insert({
			...
			clickCollect() {
				//收藏
				console.log(this.goods.itemid);
				let isExist = this.$queue.isExistPdd(this.collectKey, this.goods.itemid);
				console.log(isExist);
				if (isExist) {
					let items = [];
