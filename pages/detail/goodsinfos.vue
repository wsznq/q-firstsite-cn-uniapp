    ...
    <view class="s-page-wrapper">
        <view class="index-goods" v-if="loadGoods">
            ...
            <view class="goods_info" style="padding-top: 6px">
                ...
                <view style="padding: 8px">
                    <image v-if="goods.shoptype=='1'" style="height: 14px;width:12px;margin-top: 4px;margin-right: 4px"
        ...
        <view class="goods-list" style="margin-bottom: 60px" v-if="couponlist.length > 0">
            <orange-goods-card-home v-for="(item,index) in couponlist" :index="index%2" :tkmoney='"￥"+item.tkmoney'
                                    :itemid="item.itemid"
                                    :logo="item.shoptype=='1'?logo:taobao" :is-invitation="isInvitation"
    ...
    export default {
        ...
        onShow: function () {
            ...
            if (relation_id) {
                ...
                this.$queue.setData("erweima",
                    "https://api.qrserver.com/v1/create-qr-code/?size=150x150&data="+this.$Url+"/pages/detail/goodsinfo?itemid=" +
        ...
        methods: {
            ...
            loadGoodsInfo: function () {
				...
				let apiUrl = this.$Api.goods.iteminfo;
				this.$Request.get(apiUrl,{
                    ...
                    if (res.Code === 1) {
                        ...
                        that.goods.starttime = this.formatDate(new Date(that.starttime * 1000));
                        that.goods.endtime = this.formatDate(new Date(that.endtime * 1000));
            ...
            copyHref() {
                ...
                if (relation_id) {
                    uni.setClipboardData({
                        data: this.$Url+"/pages/detail/goodsinfo?itemid=" + this.itemid + "&relation_id=" + relation_id,
                        success: (r => {
                            this.$queue.showToast("复制成功")
                        }),
                    })
                    ...
                    uni.setClipboardData({
                        data: this.$Url+"/pages/detail/goodsinfo?itemid=" + this.itemid,
                        success: (r => {
                            this.$queue.showToast("复制成功")
                        }),
                    })
                    ...
                    if (!this.poster.finalPath) {
                        const d = await getSharePoster({
                            ...
                            qrCodeArray: ({
                                ...
                                if (relation_id) {
                                    return [{
                                        text: this.$Url+"/pages/detail/goodsinfo?itemid=" + this.itemid + "&relation_id=" +
                                            relation_id,
                                    ...
                                    return [{
                                        text: this.$Url+"/pages/detail/goodsinfo?itemid=" + this.itemid,
                                        size: bgObj.width * 0.2,
                                        dx: bgObj.width * 0.05,
                                        dy: bgObj.height - bgObj.width * 0.25
                                    }]
