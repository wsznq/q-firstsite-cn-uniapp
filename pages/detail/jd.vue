		...
		<view class="index-goods" v-if="loadGoods">
			...
			<view class="goods_info" style="padding-top: 6px;padding-bottom: 10px">
				<view style="padding: 8px">
					<image style="height: 14px;width:12px;margin-top: 4px;margin-right: 4px" src="../../static/img/jd.png"></image>
		...
		methods: {
			...
			loadGoodsInfo: function() {
				...
				this.$Request.get(this.$Api.goods.itemurl_jd,{
					...
					if (res.Code === 1) {
						that.goods = this.data;
						this.coupon_click_url = res.Message;
						this.erweima = this.coupon_click_url;
						...
						that.grade = grade;
						try {
							let isExist = this.$queue.isExistJd(this.collectKey, that.goods.itemid);
							if (isExist === true) {
							...
							let isExists = this.$queue.isExistJd(this.footprintKey, that.goods.itemid);
							if (isExists === false) {
								this.$queue.insert({
			...
			clickCollect() {
				//收藏
				let isExist = this.$queue.isExistJd(this.collectKey, this.goods.itemid);
				if (isExist) {
					let items = [];
