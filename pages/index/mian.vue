	...
	<view>
		...
		<view v-if="selectId == 2" style="padding-top: 72px;height: 100vh;background: #FFFFFF;">
			<view style="background: #FFFFFF;text-align: center;padding: 16px;color: #FC3F78;font-size: 20px;">
				...
				省钱流程
				<view>▼</view>
				<view style="font-size: 14px;color: #FF6600;">进入淘宝购物车选择任意商品点击进入详情</view>
				<view>▼</view>
				
				<!-- #ifdef H5 -->
				<view style="font-size: 14px;color: #FF6600;">长按复制商品链接</view>
				<view>▼</view>
				<view style="font-size: 14px;color: #FF6600;">发送至掌上优惠公众号</view>
				<view>▼</view>
				...
				<view>▼</view>
				<view style="font-size: 14px;color: #FF6600;">打开掌上优惠APP</view>
				<view>▼</view>
				...
				<view class="index-coupon has-bg-white has-pd-10 top_30">
					<view class="goods-list" v-if="couponlist.length > 0">
						<orange-goods-news-list v-for="(item, index) in couponlist" :key="index" :tkmoney="'返现 ￥' + item.tkmoney" :itemid="item.itemid"
						 :is-invitation="isInvitation" :logo="item.shoptype === '1' ? logo : taobao" :itempic="item.itempic + '_310x310.jpg'"
	...
	export default {
		...
		methods: {
			...
			loadCouponList: function(type) {
				this.loadingType = 1;
				this.$Request.get('/api/low_price_Pinkage_data/apikey/maxd/type/' + this.type + '/min_id/' + this.min_id).then(res => {
					this.loadingType = 0;
					if (res.code === 1) {
						if (this.page === 1) {
							this.couponlist = [];
