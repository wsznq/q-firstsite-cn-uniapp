...
<script>
	...
	export default {
		...
		methods: {
			...
			authorized() {
				// #ifdef APP-PLUS
				// plug.taobaoOauth({"url": "https://oauth.taobao.com/authorize?response_type=code&client_id=27742632&redirect_uri=https://www.firstsite.cn/pages/member/publisher&state=1&view=wap"}, ret => {
				//     console.error(ret);
				//     this.getUserInfoss(ret.code);
				// });
				let userId = this.$queue.getData("userId");
				let token = this.$queue.getData("token");
				plus.runtime.openURL(this.$Url + "/pages/member/publisher?uid=" + userId + "&token=" + token,
					function(
						res) {});
				// #endif
